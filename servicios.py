...
def cargarservicio(lista, treeservicio, entserfac):
    try:
        ...
        if iter != None:
            ...
            for j in range(3):
                if j == 0:
                    entserfac.set_text(str(listserv[j]))
                lista[j].set_text(str(listserv[j]))
    except:
        print('error carga servicioooo')
...
def nombreplato(id):
    try:
        bbdd.cur.execute('select servicio from servicios where Id = ?', (id,))
        listado = bbdd.cur.fetchone()
        bbdd.conexion.commit()
        return listado
